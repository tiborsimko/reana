...
if kubectl -n "${kubernetes_namespace}" get "deployment/${resource_prefix}-keycloak" &>/dev/null; then
    ...
    if [ -z "${keycloak_user_id}" ]; then
        # Keycloak's default user profile requires first and family names,
        # and its verify-profile required action would otherwise interpose
        # the "Update Account Information" page at first login. Derive them
        # from the first and last non-empty dot/underscore/hyphen-separated
        # tokens of the email's local part: "john.doe@example.org" gives
        # "John" and "Doe"; single-token local parts reuse the same value
        # twice, and "Admin" covers local parts with no usable token.
        admin_local_part=${admin_email%%@*}
        admin_first_name=$(
            printf '%s' "${admin_local_part}" |
                awk -F'[._-]' '{
                    for (i = 1; i <= NF; i++) if ($i != "") {
                        print toupper(substr($i, 1, 1)) substr($i, 2)
                        exit
                    }
                }'
        )
        ...
        admin_last_name=${admin_last_name:-${admin_first_name}}
        keycloak_user_id=$(
            kubectl -n "${kubernetes_namespace}" exec "deployment/${resource_prefix}-keycloak" -- \
